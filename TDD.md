...
Toda cinemática nueva (`XxxSequencer : CinematicSequencerBase`, escena aditiva en `Assets/Scenes/Cinematics/Aditivas/` + Timeline en `Assets/Timeline/`) se diseña **antes** de tocar código o el Editor, en tres bloques, como el guion técnico de una obra de teatro:

1. **Materiales** — todo lo que la secuencia necesita para existir: prefabs de actores, escena aditiva, Timeline (`.playable`), `Volume` URP de post-proceso dedicado, clips de audio (voz/SFX/música), VFX a reutilizar vía `VfxPoolService` (nunca instanciar VFX de un solo uso sin pool — ver § 12). Se anota explícitamente qué ya existe en el proyecto (con ruta) y qué falta por crear o buscar.
