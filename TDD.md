...
- **INC-077 — cámara de diálogo grupal embebida en geometría (pasillo de mazmorra, previo al puzle de botones, tras la misión "Operación Avestruz").** Reportado con Liam como personaje activo (`ActiveCharacterSwapper`): al iniciar el diálogo grupal con Will como speaker, la cámara terminaba pegada a la esquina de un pilar junto a una puerta, mostrando un primerísimo plano de piedra en vez de encuadrar al grupo. Causa raíz en `DialogueCinematicController.CalculateCameraPosition()` (rama `_isGroupConversation`, `Assets/Scripts/Dialogue/DialogueCinematicController.cs`): el único chequeo de obstrucción era un `SphereCast` en línea recta a la altura de `groupLookAtHeight` (`GroupCameraFreeDistance`), y cuando detectaba pared cercana compensaba subiendo la altura y aplicando un clamp mínimo de 1.5m **sin volver a comprobar si esa posición final seguía dentro de geometría** — el mismo fallo de "vano más estrecho que el hueco entre marcos" ya documentado y arreglado para el modo 1:1 (`CameraPositionIsEmbedded`, ver comentario en el código ~línea 1941), pero la rama grupal hacía `return` antes de llegar a ese chequeo. Contribuye `ActiveCharacterSwapper.TeleportPlayer()`: al cambiar de personaje activo teletransporta el controller instantáneamente a la posición exacta donde estaba el NPC objetivo (sin comprobación de clearance), así que si Liam (NPC con IA) estaba pegado a una pared/puerta en un pasillo estrecho al hacer el swap, el diálogo grupal arrancaba con ese punto como ancla de cámara. **Arreglado** (Agosto 2026): se añadió a la rama grupal de `CalculateCameraPosition()` la misma verificación final de solape (`CameraPositionIsEmbedded` + retroceso lateral en pasos cortos, tope de 6 pasos) que ya tenía el modo 1:1, con elevación de emergencia como último recurso si el retroceso lateral no basta. No se tocó `ActiveCharacterSwapper.TeleportPlayer()` — si el bug reaparece en otra variante (p.ej. cámara 1:1 embebida justo tras un swap), revisar ahí una validación de clearance post-teleport (`NavMesh.SamplePosition` o similar) como mitigación adicional, pendiente si se necesita.
