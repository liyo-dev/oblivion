...
$problematicEvents = @(
    ...
    "BATTLE_START:Golem_1_received",
    "BATTLE_START:Demon_1_received"
)
...
foreach ($preset in $presets) {
    ...
    if ($hasProblems) {
        $totalProblems++
        Write-Host "⚠️  $($preset.Name)" -ForegroundColor Yellow
        foreach ($problem in $problems) {
            Write-Host "   - Tiene flag: $problem" -ForegroundColor Red
        }
        Write-Host ""
    } else {
        Write-Host "✅ $($preset.Name)" -ForegroundColor Green
    }
}
...
if ($totalProblems -gt 0) {
    ...
    Write-Host ""
    Write-Host "Para limpiar un preset:" -ForegroundColor Cyan
    Write-Host "1. Abrir el archivo .asset en editor de texto" -ForegroundColor Gray
    Write-Host "2. Buscar y eliminar lineas con _received" -ForegroundColor Gray
    Write-Host "3. Guardar y recargar en Unity" -ForegroundColor Gray
}
